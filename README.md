...
[機能、特徴]  
  ・現在、以下の主にメガドライブ系音源チップのエミュレーションによる再生が可能です。  
    AY8910 , YM2612 , SN76489 , RF5C164 , PWM , C140 , OKIM6258 , OKIM6295 , SEGAPCM , YM2151 , YM2203 , YM2413 , YM2608 , YM2610/B , HuC6280  
  
  ・現在、以下の鍵盤表示が可能です。  
    AY8910 , YM2612 , SN76489 , RF5C164 , C140 , SEGAPCM , YM2151 , YM2203 , YM2608 , YM2610/B , HuC6280  
  ...
  パラメーターの形式はオプション設定から変更可能です。  
    FMP7 , MDX , MUSIC LALF , NRTDRV , HuSIC , MML2VGM , .TFIに対応しており、.TFIを選んだ場合はクリップボードの代わりにファイルに出力します。  
...
[MIDI鍵盤のすゝめ]  
  ...
  ・画面  
    ...
    ９．音色データ(6Ch分)  
      ...
      ・パラメータをクリックすることで、そのパラメータの変更ができます。(TBD)  
      ・「LOG」の隣の「♪」をクリックすることで、そのチャンネルのノートログがクリアされます。  
      ・LOGをクリックすることで、MMLデータをクリップボードに設定します。(TBD)  
      
  ・MIDI鍵盤からの操作  
    CC:97(DATA DEC)  
      YM2612の1Chの音色を全てのチャンネルにコピーします。(選択状況無視)  
    CC:96(DATA INC)  
      直近のログをひとつだけ削除します。(弾き間違い等を取り消す機能)(TBD)  
    CC:66(SOSTENUTO)  
      MONOモード時のみ、選択行のログをMMLにしクリップボードに設定します。(TBD)  
