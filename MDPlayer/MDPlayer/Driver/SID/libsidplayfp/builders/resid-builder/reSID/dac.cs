...
namespace Driver.libsidplayfp.builders.resid_builder.reSID
{
    public class dac
    {
        ...
        public void build_dac_table(ref UInt16[] dac, Int32 bits, double _2R_div_R, bool term)
        {
            ...
            for (int set_bit = 0; set_bit < bits; set_bit++)
            {
                ...
                double Rn = term ?        // Rn = 2R for correct termination,
                  _2R : float.PositiveInfinity;// INFINITY;         // INFINITY for missing termination.

                // Calculate DAC "tail" resistance by repeated parallel substitution.
                for (bit = 0; bit < set_bit; bit++)
                {
                    if (Rn == float.PositiveInfinity)// INFINITY)
                    {
                        Rn = R + _2R;
                    }
                    else
