...
namespace MDPlayer.Driver.ZGM.ZgmChip
{
    public class YM2609 : ZgmChip
    {
        ...
        {
            //chipRegister.YM2609SetRegister(od, Audio.DriverSeqCounter, Index, 0, vgmBuf[vgmAdr + 1], vgmBuf[vgmAdr + 2]);
            vgmAdr += 3;
        }

        private void SendPort1(byte od, ref uint vgmAdr)
        {
            //chipRegister.YM2609SetRegister(od, Audio.DriverSeqCounter, Index, 1, vgmBuf[vgmAdr + 1], vgmBuf[vgmAdr + 2]);
            vgmAdr += 3;
        }

        private void SendPort2(byte od, ref uint vgmAdr)
        {
            //chipRegister.YM2609SetRegister(od, Audio.DriverSeqCounter, Index, 2, vgmBuf[vgmAdr + 1], vgmBuf[vgmAdr + 2]);
            vgmAdr += 3;
        }

        private void SendPort3(byte od, ref uint vgmAdr)
        {
            //chipRegister.YM2609SetRegister(od, Audio.DriverSeqCounter, Index, 3, vgmBuf[vgmAdr + 1], vgmBuf[vgmAdr + 2]);
            vgmAdr += 3;
        }
