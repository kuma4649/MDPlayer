...
namespace MDPlayer.Driver.NDP
{
    public class NDP : baseDriver
    {
        ...
        {
            //log.Write("\r\n INTRPT(C009H)");
            z80.Registers.PC = 0xC009;
            z80.Registers.SP = unchecked((short)0xf380);
            z80.Continue();
            //DebugRegisters(z80);

            //log.Write("\r\n RDSTAT(C033H)");
            z80.Registers.PC = 0xC033;
            z80.Registers.SP = unchecked((short)0xf380);
            z80.Continue();
            byte PLAYFG = (byte)(z80.Registers.A & 0xf);
            if (PLAYFG == 0x0) Stopped = true;

            //log.Write("\r\n RDENDT(C036H)");
            z80.Registers.PC = 0xC036;
            z80.Registers.SP = unchecked((short)0xf380);
            z80.Continue();
            PLAYFG = (byte)(z80.Registers.A & 0xf);
            PLAYFG = (byte)(
                ((z80.Memory[0x4000] | z80.Memory[0x4001]) == 0 ? 1 : (PLAYFG & 1))
                | ((z80.Memory[0x4002] | z80.Memory[0x4003]) == 0 ? 2 : (PLAYFG & 2))
                | ((z80.Memory[0x4004] | z80.Memory[0x4005]) == 0 ? 4 : (PLAYFG & 4))
                | ((z80.Memory[0x4006] | z80.Memory[0x4007]) == 0 ? 8 : (PLAYFG & 8))
                );
            if (PLAYFG == 0xf) Stopped = true;

            //log.Write("\r\n RDLOOP(C039H)");
            z80.Registers.PC = 0xC039;
            z80.Registers.SP = unchecked((short)0xf380);
            z80.Continue();
            uint a = (uint)z80.Registers.A;
        ...
        private void Run(byte[] vgmBuf)
        {
            ...
            //byte[] mgsdata = vgmBuf;
            //ushort dataAdr = 0x4000;

            log.Write(LogLevel.Trace, "\r\nKSSPLY(C051H)");
            z80.Registers.PC = 0xC051;
            z80.Registers.SP = unchecked((short)0xf380);
            z80.Continue();
        ...
        {
            string nimo = Z80Opcode.GetNimo(z80.Memory[z80.Registers.PC], z80.Memory[z80.Registers.PC + 1], z80.Memory[z80.Registers.PC + 2]);
            log.Write(LogLevel.Trace, "nimo:{7} Reg PC:{0:x04} AF:{1:x04} BC:{2:x04} DE:{3:x04} HL:{4:x04} IX:{5:x04} IY:{6:x04}"
                , z80.Registers.PC
                , z80.Registers.AF, z80.Registers.BC, z80.Registers.DE, z80.Registers.HL
                , z80.Registers.IX, z80.Registers.IY,nimo);
        }
        ...
        private void CallEXTBIO(BeforeInstructionFetchEventArgs args, IZ80Processor z80)
        {
            ...
            {
                case 0x00:
                    //log.Write(" EXTBIO broadcast");
                    switch (function)
                    {
                        case 0:
                            break;
                        default:
                            log.Write(LogLevel.Trace, " EXTBIO broadcast Unknown function");
                            break;
                    }
                    break;
                case 0x04:
